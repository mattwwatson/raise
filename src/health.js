/**
 * Asking a port whether nmmon is behind it.
 *
 ...
 * NMMON_HOME holds the port while leaving no record at all. Both states read as
...
/**
 * Is there an nmmon on this port right now?
 *
 ...
 * @returns {Promise<{pid: number} | null>} null for anything that is not nmmon
 */
export async function probeHealth(port, { timeoutMs = DEFAULT_TIMEOUT_MS, fetchImpl = fetch } = {}) {
  ...
  try {
    ...
    // Refused, timed out, not JSON: all of them mean "no nmmon here".
    return null;
  } finally {
    clearTimeout(timer);
  }
