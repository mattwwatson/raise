...
**Planning happens in a tracker that is not public.** The specifications are not: every item
has one in [docs/tasks/](docs/tasks/), in the repository, saying what was built and why. If you
want to know how something got the way it is, that and `AGENTS.md` are the two places to look.
You are not expected to write a spec for a contribution - that is the maintainer's job.
